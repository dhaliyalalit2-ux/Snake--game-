...
<head>
  ...
  <style>
    body { display: flex; flex-direction: column; align-items: center; background: #222; color: white; font-family: Arial; }
    canvas { background: #000; margin-top: 20px; border: 2px solid #fff; }
    .controls { margin-top: 10px; display: grid; grid-template-columns: repeat(3, 60px); grid-gap: 10px; }
    button { padding: 10px; font-size: 18px; border-radius: 8px; border: none; background: #444; color: white; cursor: pointer; }
    button:hover { background: #666; }
    #restart { margin-top: 15px; grid-column: span 3; background: crimson; }
  </style>
</head>
<body>
  <h1>Snake Game</h1>
  <canvas id="gameCanvas" width="400" height="400"></canvas>  <div class="controls">
    ...
    function initGame() {
      ...
      game = setInterval(draw, 200); // speed adjusted here
    }
